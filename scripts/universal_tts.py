    ...
    if os.path.exists(ttnet_path):
        try:
            with open(ttnet_path, 'r', encoding='utf-8', errors='ignore') as f:
                ...
                if match: config["device_id"] = match.group(1)
        except: pass
...
async def _run_sami_tts(text, speaker, output_file, dev_id, iid):
    ...
    try:
        async with websockets.connect(ws_url, additional_headers=headers, ssl=ssl_context, open_timeout=20) as ws:
            ...
            start_msg = {
                "app_id": APP_ID, "appkey": APP_KEY, "event": "StartTask", "namespace": "TTS",
                "task_id": task_id, "message_id": task_id + "_0",
                "payload": json.dumps({
            ...
            while True:
                ...
                if isinstance(resp_raw, str):
                    ...
                    if event == "TaskFailed": 
                        return False, f"SAMI Error: {resp.get('status_text')} (Code: {resp.get('status_code')})"
                    if event == "TaskFinished": break
                else: 
